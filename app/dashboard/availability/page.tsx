...
import { Select, SelectContent, SelectValue,SelectGroup, SelectTrigger, SelectItem } from "@/components/ui/select";
import { Switch } from "@/components/ui/switch";
import { notFound } from "next/navigation";

async function getData(userId: string ){
    const data = await prisma.availaibility.findMany({
        where:{
            userId: userId
        }
    })
...
export default async function AvailabilityRoute() {
  ...
  const data = await getData(session.user?.id as string);
    ...
    <Card>
        ...
        <CardDescription>
            In this Secrion you can manage your availability    
        </CardDescription> 
        </CardHeader>
        <form action={updateAvailabilityAction}>
            <CardContent className="flex flex-col gap-y-4">
                {data.map((item: { id: string; isActive: boolean; day: string; }) => (
                    <div 
                        key={item.id} 
                        className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 items-center gap-4"
                    >
                        
                        <input type="hidden" name ={`id-${item.id}`} value={item.id} /> {/* for actions.ts */}
